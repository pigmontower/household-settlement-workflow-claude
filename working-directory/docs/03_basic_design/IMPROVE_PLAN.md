...
現状はGoogle スプレッドシートでデータを管理する方針だが、以下の観点から見直しを検討する：

- ワークフロー内でのみ利用するデータ管理に外部サービス（Google）を利用している
- これによりGoogle認証の設定・管理が必要となる
- また、スプレッドシート自体の管理コストが発生する

### 改善案

データの永続化には、Google スプレッドシートではなく **n8n の Data Tables** の利用を検討する。

#### 比較表

| 観点 | Data Tables | Google スプレッドシート |
|:---|:---|:---|
| 認証設定 | 不要 | Google OAuth認証が必要 |
| n8n内での連携 | ネイティブ対応、高速 | API呼び出しが必要 |
| 外部からのアクセス | n8n内でのみアクセス可能 | ブラウザで閲覧・編集可能 |
| 管理コスト | 低い（n8n内で完結） | 高い（外部サービス管理が必要） |

#### 採用理由

| 観点 | 判断 |
|:---|:---|
| 用途 | ワークフロー内でのデータ処理（過去データの逐次加算・集計） |
| 共有要件 | なし（個人利用のみ） |
| 外部アクセス | 不要（ワークフロー内で完結） |

#### 環境構成方針

| 環境 | 方針 |
|:---|:---|
| 開発・検証 | ローカルPC（Docker）で構築 |
| 本番 | クラウド環境（n8n Cloud / AWS等）へ移行 |
| 移行対象 | ワークフロー定義（JSONエクスポート） |
| 移行対象外 | 検証データ、実行履歴 |
| 再設定必要 | Credentials（APIキー等の認証情報） |

**移行手順**:
1. ローカル環境でワークフローを「Download」（JSON形式）
2. クラウド環境で「Import from file...」
3. Credentialsを再設定（APIキー等を再入力）

> **Status**: [検討中] Phase 2 以降の検証で Data Tables の動作を確認した上で最終決定する
